        ...
        <div class="mt-10 px-6 py-6 w-full">
          <div class="mt-5 bg-white rounded-lg border border-gray-200">
            <div class="flex items-center justify-between border-b border-gray-200">
                <h2 class="text-gray-500 px-5 py-3 text-base font-medium uppercase border-b border-gray-100">Survey List</h2>
            </div>
            <div class="xs:overflow-x-auto">
                <LoadingData v-if="isDataLoading" />
                <NoData v-else-if="countSurveys === 0" message="No Survey Available" />
                ...
                <table v-else class="w-full">
                  ...
                  <tbody>
                      <tr v-for="(survey, index) in surveys" :key="survey.id" :class="[index % 2 !== 0 ? 'bg-gray-50': '']" class="border-b border-gray-50">
                          ...
                          <td class="text-gray-700 flex items-center text-sm space-x-3 px-3 py-5">
                            <router-link :to="`/survey/${survey.id}/detail`">
                            ...
                            <svg @click.prevent="removeSurvey(survey.id)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 cursor-pointer hover:text-red-600">
            ...
            <div class="flex items-center justify-between border-t border-gray-200">
                <h2 class="text-gray-700 p-3 border-b border-gray-100">Showing <span class="text-gray-800 font-medium">{{ surveys.length }}</span> of <span class="font-medium">{{ countSurveys }}</span></h2>
                <Pagination :pagination="getPagination" :offset="3" @paginate="paginatedSurveyList(getPagination.current_page)"/>
            </div>
          </div>
        </div>
    </div>
...
<script>
    ...
    import { deleteSurvey } from '../../library/survey'
    ...
    export default {
        ...
        mounted() {
            document.title = "Survey List"
        },
        created(){
            this.paginatedSurveyList(1)
        ...
        computed: {
            ...mapGetters({
                ...
                'countSurveys': 'survey/countSurveys',
                'getPagination':'survey/getPagination'
            })
        },
        methods: {
            ...mapActions({
                'paginatedSurveyList': 'survey/paginatedSurveyList'
            }),
            removeSurvey(id){
                deleteSurvey(getToken(), {id: id}).then(() => {
                    this.paginatedSurveyList(1)
                    createToast('Survey deleted successfully', {
                        type: 'success',
                        position: 'top-center',
                        showIcon: true,
                        timeout: 1500
